...
jQuery(document).ready(function() {
	...
	if (getCookie("nls_currentab") != "") {
		jQuery("ul.options_tabs li a#" + getCookie("nls_currentab"))
			.parent()
			.addClass("currunt active");
		jQuery("ul.options_tabs li a#" + getCookie("nls_currentab")).addClass(
			"active"
		);
	...
	// // subscriber form GDPR option js
	// jQuery('#subscriber_form_gdpr').change(function() {
	// 	if (this.checked) {
	// 		jQuery('.subscriber-gdpr-option').addClass('active');
	// 	} else {
	// 		jQuery('.subscriber-gdpr-option').removeClass('active');
	// 	}
	// });
 ...
 */
function wl_copy_to_clip_board( wl_nl_template_id='NLT1' ) {
	// alert( wl_nl_template_id );
	var copyText = document.getElementById(wl_nl_template_id);
	copyText.select();
