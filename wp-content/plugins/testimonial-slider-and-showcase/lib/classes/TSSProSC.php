	...
	class TSSProSC {
			...
			if ( $scID && ! is_null( get_post( $scID ) ) ) {
				...
				if ( $hasPagination && ! $isCarousel ) {
					...
					if ( $paginationType == 'pagination' ) {
						$htmlUtility .= TSSPro()->pagination( $tssQuery->max_num_pages, $tssArgs['posts_per_page'] );
		...
		private function metas( array $meta, $pID = null ) {
			return array(
				...
				'isotopeShowAll'      => $meta['tss_isotope_filter_show_all'][0],
				'isotopeSearchFilter' => $meta['tss_isotope_search_filtering'][0],
			);
		}
